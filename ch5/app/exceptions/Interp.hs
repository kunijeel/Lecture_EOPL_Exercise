...
apply_cont (Unop_Arg_Cont op cont) val = apply_cont cont (apply_unop op val)

apply_cont (Rator_Cont rand env cont) ratorVal = value_of_k rand env (Rand_Cont ratorVal cont)

apply_cont (Rand_Cont ratorVal cont) randVal = 
  apply_procedure_k (expval_proc ratorVal) randVal cont
...
apply_handler val (End_Cont) = error ("Uncaught exception: " ++ show val)
...
value_of_k (Unary_Exp op exp1) env cont = value_of_k exp1 env (Unop_Arg_Cont op cont)
  
value_of_k (If_Exp exp1 exp2 exp3) env cont = value_of_k exp1 env (If_Test_Cont exp2 exp3 env cont)

value_of_k (Let_Exp var exp1 body) env cont = value_of_k exp1 env (Let_Exp_Cont var body env cont)
...
value_of_k (Proc_Exp var body) env cont = apply_cont cont (Proc_Val (procedure var body env))

value_of_k (Call_Exp rator rand) env cont = value_of_k rator env (Rator_Cont rand env cont)
  
value_of_k (Try_Exp exp var handler_exp) env cont = 
  value_of_k exp env (Try_Cont var handler_exp env cont)

value_of_k (Raise_Exp exp) env cont = value_of_k exp env (Raise1_Cont cont)
...
apply_procedure_k (Procedure var body env) arg cont = 
  value_of_k body (extend_env var arg env) cont
